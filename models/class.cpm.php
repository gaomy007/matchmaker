...
class cpm {
    ...
    function getUnreadedNumber(){
       global $mysqli;
      $sql = "SELECT COUNT(*) FROM messages WHERE `to` = '".$this->userid."' && `to_viewed` = '0' && `to_deleted` = '0'" ;
    ...
    function getusername($userid) {
         global $mysqli;
        $sql = "SELECT display_name FROM uc_users WHERE `id` = '".$userid."' LIMIT 1";
    ...
    function getuserid($username) {
         global $mysqli;
        $sql = "SELECT id FROM uc_users WHERE `display_name` = '".$username."' LIMIT 1";
