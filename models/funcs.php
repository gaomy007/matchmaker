...
function hasRpid()
{
  $rpid=NULL;
	global $loggedInUser,$mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT rpid
		FROM ".$db_table_prefix."users
		WHERE
		id = ?");
	$stmt->bind_param("i", $loggedInUser->user_id);	
...
function uploadImg($photo){
 ...
 $savefolder = 'userImg';		// folder for upload
 $max_size = 5000;			// maxim size for image file, in KiloBytes
...
$name=explode(".", $name);
  ...
  if (in_array($type, $allowtype)) {
    // check its size
	if ($photo['size']<=$max_size*1000) {
      // if no errors
      if ($photo['error'] == 0) {
        $thefile = $savefolder . "/" .time().$photo['name'];
       // echo $thefile;
        // if the file can`t be uploaded, return a message
        if (!move_uploaded_file ($photo['tmp_name'], $thefile)) {
          $rezultat = NULL;
        }
        else {
...
$image="userImg/default.png";
    ...
    return '<img src="' . $image . '" width="' . $imgWidth . '" height="' . $imgHeight . '" id="'.'blah'.'" />';
}
...
function lookup($string){
   ...
   //print_r($response['results'][0]['address_components'][4]['long_name']);
...
function page($page,$total,$phpfile,$pagesize=5,$pagelen=5,$mode){
    ...
    $pagecode='<nav style="width: 420px;">
    <ul class="pagination">';
    if($pages!=1)
    
    //如果是第一页，则不显示第一页和上一页的连接
    if($page!=1){
        $pagecode.="<li><a href=\"{$phpfile}?page=1&sort={$mode}\">&lt;&lt;</a></li>";//第一页
        $pagecode.="<li><a href=\"{$phpfile}?page=".($page-1)."&sort={$mode}\">&lt;</a></li>";//上一页
    }
    //分页数大于页码个数时可以偏移
    if($pages>$pagelen){
        //如果当前页小于等于左偏移
    ...
    for($i=$init;$i<=$max;$i++){
        if($i==$page){
            ...
            $pagecode.="<li><a href=\"{$phpfile}?page={$i}&sort={$mode}\">$i</a></li>";
        }
    }
    if($page!=$pages){
        $pagecode.="<li><a href=\"{$phpfile}?page=".($page+1)."&sort={$mode}\">&gt;</a></li>";//下一页
        $pagecode.="<li><a href=\"{$phpfile}?page={$pages}&sort={$mode}\">&gt;&gt;</a></li>";//最后一页
    }
    //$pagecode.="<span>  go</span><input type=\"text\" size=\"3\" onkeydown=\"if(event.keyCode==13) {window.location.href='{$phpfile}?page='+this.value; return false;}\" /></div>";
    $pagecode.="</ul></div>";
